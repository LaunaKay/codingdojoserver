...
class M_Dashboard extends CI_Model
{
    ...
    {
        return $this->db->query("SELECT task,DATE_FORMAT(time, '%h:%i %p') as time,status,tasks.id
                                    FROM tasks
    ...
    {
        return $this->db->query("SELECT task,DATE_FORMAT(time, '%h:%i:%p') as time,DATE_FORMAT(date, '%M %D') as date,tasks.id
                                    FROM tasks
                                    LEFT JOIN users ON users.id = tasks.user_id
                                    WHERE tasks.date > DATE(NOW())
                                    AND tasks.user_id = '{$id}'")->result_array();
